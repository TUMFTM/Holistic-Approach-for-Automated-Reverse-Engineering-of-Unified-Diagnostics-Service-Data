...
def did_discovery(servers,
    ...
    for server in servers:
        ...
        try:
            ...
            for did in possible_dids_server:
                if print_results:
                    print("\rProbing did 0x{1:04x} for server 0x{0:04x}: found {2} - {3:.2f}% complete."
                          .format(server.id, did, len(server.parameters), ((did / max(possible_dids)) * 100)), end="")
...
def __did_discovery_wrapper(config_file_path, 
    ...
    # Reset discovery complete flag for all servers if reset_did_discovery is set to true
    if reset_did_discovery:
        for server in car.servers:
            server.discovery_complete_flag = False
            server.first_unchecked_did_in_did_discovery = 0
            server.did_discovery_time_seconds = 0
            server.parameters = []
        return
...
if __name__ == "__main__":
    ...
    __did_discovery_wrapper(args.config_file_path, args.car_model_path, args.reset_did_discovery, args.activate_logging_flag)

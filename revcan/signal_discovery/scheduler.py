...
from revcan.signal_discovery.dids import (
    RequestID,
    DidRequest,
    RequestList,
    DidRequestDatabase,
...
class Scheduler:
    ...
    def __init__(self):
        ...
        self.buffer_list: list[DidRequest] = []
    ...
    def load_requests(self, path, maximum_payload_length=-1):
        ...
        if load_path[-3:] == ".db":
            ...
            self.request_list.fill_request_list_from_database_files(
                load_path, want_payload_history=False
            )
    ...
    def save_data(self, save_name, export_csv=False):
        """
        ...
        save_string = "database/" + save_name + ".db"
        save_database_file = os.path.join(self.script_directory, save_string)
        with DidRequestDatabase(save_database_file) as database:
        ...
        if self.GUI_mode:
            ...
            for subset_number, subset in enumerate(self.subset_lists):
                ...
                if answer:
                    print(len(subset.request_list))
                    path_string = (
                        "csv/all_dids_driving_20230906/"
                        + output_name
                        + "_subset_"
                        + str(subset_number)
                        + ".csv"
                    )
                    self.csv_filepath = os.path.join(self.script_directory, path_string)
    ...
    def add_requests_to_buffer(self, subset_number):
        ...
        for request in self.subset_lists[subset_number].request_list:
            ...
            if (
                ...
                request_ids = RequestID(0x17FC0076, 0x17FE0076, 0x346)
                if request.ids == request_ids:
                    print(request)
                    print("Lenkradwinkel in buffer geschrieben")

    def append_debug_history(self, request: DidRequest, new_interval):
        """

...
class Parser:
    ...
    def read_message(self, data_bytes):
        logger.debug(" Reading TCP Message")
        self.rx_buffer += data_bytes
        ...
        if self._state == Parser.ParserState.READ_INVERSE_PROTOCOL_VERSION:
            if len(self.rx_buffer) >= 1:
                ...
                if inverse_protocol_version != (0xFF ^ self.protocol_version):
                    ...
                    return GenericDoIPNegativeAcknowledge(
                        GenericDoIPNegativeAcknowledge.NackCode.IncorrectProtocolVersionInverse
                    )
                else:
...
class DoIPClient:
    ...
    :type initial_ecu_ip_address: str
    :param initial_ecu_logical_address: The logical address of the target ECU. This should be an integer. According to the
    ...
    def __init__(
        ...
        self._ecu_logical_address = initial_ecu_logical_address
        ...
        if self._activation_type is not None:
            logger.warning("Requesting activation")
            ...
            if type(response) == GenericDoIPNegativeAcknowledge:
                ...
                if (
                    ...
                    try:
                        ...
                        pass
        raise TimeoutError("ECU failed to respond in time", timeout)
    ...
    def request_activation(
        ...
        logger.debug("Requesting activation in function")
        self.send_doip_message(message, disable_retry=disable_retry)
        while True:
            result = self.read_doip(self.timeout)
    ...
    def send_diagnostic_no_response(self, diagnostic_payload):
        """Send a raw diagnostic payload (ie: UDS) to the ECU.
        same as send_diagnostic() but doesnt return the response
        :param diagnostic_payload: UDS payload to transmit to the ECU
        :type diagnostic_payload: bytearray
        :raises IOError: DoIP negative acknowledgement received
        """
        ...
        self.send_doip_message(message, disable_retry=True)

    def receive_diagnostic(self, timeout: float = None):
    ...
    def change_ecu_logical_address(self, new_logical_address):
        """Change the logical address of the ECU.

        :param new_logical_address: New logical address for the ECU
        :type new_logical_address: int
        """
        self._ecu_logical_address = new_logical_address

from revcan.signal_discovery.utils.can_actions import DEFAULT_INTERFACE
...
class IsoTp:
    ...
    def __init__(self, arb_id_request, arb_id_response, blacklist_IDs=None, bus=None):
    ...
    def send_message(self, data, arbitration_id, force_extended=False):
        ...
        msg = can.Message(
            arbitration_id=arbitration_id, data=data, is_extended_id=is_extended
        )
        self.bus.send(msg)
    ...
    def encode_fc(self, flow_status, block_size, st_min):
        ...
        """
        return [
            (self.FC_FRAME_ID << 4) | flow_status,
            block_size,
            st_min,
            0,
            0,
    ...
    def indication(self, wait_window=None, trim_padding=True, first_frame_only=False):
        ...
        while True:
            ...
            if msg is not None:
                # Returns None if the arbitration id is in the Blacklist
                if self.blacklist_IDs is not None:
                    if msg.arbitration_id in self.blacklist_IDs:
                        print("ID in Blacklist")
                        return None
            ...
            while number_of_frames_left_to_send > 0:
                ...
                while not receiver_is_ready:
                    ...
                    if fs == self.FC_FS_WAIT:
                        ...
                        if (
                            number_of_frames_left_to_send
                            < number_of_frames_left_to_send_in_block
                            or block_size == 0
                        ):
                            number_of_frames_left_to_send_in_block = (
                                number_of_frames_left_to_send

...
class Client:
    ...
    def change_session(
        ...
        response = self.send_request(req, self.timeout)
        if response is None:
            return None
    ...
    def read_data_by_identifier(
        self, didlist: Union[int, List[int]], timeout: Union[float, None] = None
        ...
        """
        didlist = services.ReadDataByIdentifier.validate_didlist_input(didlist)
        req = services.ReadDataByIdentifier.make_request(didlist=didlist)
        ...
        if timeout is None:
            timeout = (
                self.timeout
            )  # enabls to use a diffenrent timeout for each requestd
        ...
        self.logger.debug("Response positive: %s" % response.positive)
        self.logger.debug("Response data: %s" % response.data)
    ...
    @standard_error_management
    def read_data_by_identifier_no_return(
        self, didlist: Union[int, List[int]]
    ) -> Optional[services.ReadDataByIdentifier.InterpretedResponse]:
        """
        Requests a value associated with a data identifier (DID) through the :ref:`ReadDataByIdentifier<ReadDataByIdentifier>` service.
        ...
        self.send_request_no_response(req)
    ...
    def communication_control(
        ...
        self.logger.info(
            "%s - ControlType=0x%02x (%s) - Sending request with a CommunicationType byte of 0x%02x (%s)"
            % (
    ...
    def dynamically_define_did(
        ...
        if did_definition.is_by_source_did():
            ...
            subfunction = (
                services.DynamicallyDefineDataIdentifier.Subfunction.defineByMemoryAddress
            )
    ...
    def do_clear_dynamically_defined_did(
        ...
        subfunction = (
            services.DynamicallyDefineDataIdentifier.Subfunction.clearDynamicallyDefinedDataIdentifier
        )
    ...
    def send_request(self, request: Request, timeout: float = -1) -> Optional[Response]:
        if request.service is None:
            print(ValueError("Request has no service"))

        if self.ecu_logical_address is not None:
            self.conn.change_address(self.ecu_logical_address)
        ...
        self.logger.debug("\n", time.time(), " Sending request to server")
        ...
        while not done_receiving and not timed_out:
            ...
            try:
                if not respect_overall_timeout or (
                    ...
                    and time.time() + single_request_timeout < overall_timeout_time
                ):
            ...
            if not response.valid:
                print(InvalidResponseException(response))
                response.positive = False
                return response
            ...
            if not response.positive:
                ...
                if (
                    ...
                    self.logger.debug("Response Pending")
                    done_receiving = False
                ...
                elif response.code == Response.Code.RequestOutOfRange:
                    response.positive = False
    ...
    def send_request_no_response(self, request: Request):
        if request.service is None:
            print(ValueError("Request has no service"))
        ...
        self.conn.send_no_response(payload)

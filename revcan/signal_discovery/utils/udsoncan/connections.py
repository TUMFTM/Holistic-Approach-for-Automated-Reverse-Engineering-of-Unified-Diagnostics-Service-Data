...
class BaseConnection(ABC):
    ...
    def send(self, data):
        ...
        self.logger.debug(
            "Sending %d bytes : [%s]" % (len(payload), binascii.hexlify(payload))
        )
    ...
    def send_no_response(self, data):
        ...
        self.specific_send_no_response(payload)

    def wait_frame(self, timeout: float = 2, exception=False):
        ...
        if frame is not None:
            self.logger.debug(
                "Received %d bytes : [%s]" % (len(frame), binascii.hexlify(frame))
            )
        return frame

    def wait_frame_complete_message(self, timeout: float = 2, exception=False):
        """Waits for the reception of a frame of data from the underlying transport protocol
        ...
        :returns: Received message (complete message, not just a frame; uds message is in .user_data attribute)
        :rtype: bytes or None
        """
        try:
            frame = self.specific_wait_frame_complete_message(timeout=timeout)
        except Exception as e:
            self.logger.debug(
                "No data received: [%s] - %s " % (e.__class__.__name__, str(e))
            )
        ...
        if frame is not None:
            frame_bytes = bytes(frame.user_data)
            self.logger.debug(
                "Received %d bytes : [%s]"
                % (len(frame_bytes), binascii.hexlify(frame_bytes))
            )
        return frame

...
def auto_blacklist(bus, duration, classifier_function, print_results):
    ...
    while time.time() < end_time:
        if print_results:
            ...
            print(
                "\r{0:> 5.1f} seconds left, {1} found".format(time_left, num_matches),
                end="",
            )
            stdout.flush()
        # Receive message
...
class CanActions:
    ...
    def send(
        self, data, arb_id=None, is_extended=None, is_error=False, is_remote=False
    ):
        if len(data) > 8:
    ...
    def bruteforce_arbitration_id(
        self, data, callback, min_id, max_id, callback_end=None
    ):
        # Set limits
        if min_id is None:
